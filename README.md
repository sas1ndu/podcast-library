...
- R2: uploaded audio, public recording metadata, and transcripts the user explicitly publishes.
...
The main library is a public cross-device catalogue built from the R2 `audio/` object listing. Every visitor can discover and play every uploaded recording. Personal subjects, bookmarks, progress, settings, and unpublished transcripts remain in the current browser. Anyone with a link containing a published transcript reference can read that transcript.
...
- `publicLibrary`: enable the shared catalogue and set pagination/item safeguards.
...
The Worker upload endpoint and recording catalogue are anonymous and open. File-size/MIME/schema checks are resource/transport safeguards, not authentication, quotas, malware scanning, Turnstile, or rate limiting. A discovered endpoint can be abused and may incur storage/egress costs. Recording titles, filenames, subject labels, and audio are public. Unlisted transcript links are not private authentication. Do not upload confidential recordings.
